...
class OABoardModel: NSObject {
    ...
    class func dictToModel(dic:[String: AnyObject]) -> OABoardModel {
        let model = OABoardModel(dict: dic)
        return model
    }
    ...
    override func setValue(_ value: Any?, forUndefinedKey key: String) {
        print("---OABoardModel-forUndefinedKey---\(key)值为空")
    }
    
    //MARK:- 数组转model
    class func listToModel(list:[[String: AnyObject]]) -> [OABoardModel] {
        var models = [OABoardModel]()
        for dict in list {
            models.append(OABoardModel(dict: dict))
        }
        return models
    }
