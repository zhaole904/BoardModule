...
@objcMembers
class OABoardInfoModel: NSObject {
    var branchLevel: String = ""
    var branchs: [BoardBranchsoModel] = []
    
    class func dictToModel(dic:[String: AnyObject]) -> OABoardInfoModel {
        let model = OABoardInfoModel(dict: dic)
        return model
    }
...
//        setValuesForKeys(dict)
    ...
    //MARK:- 数组转model
    class func listToModel(list:[[String: AnyObject]]) -> [OABoardInfoModel] {
        
        var models = [OABoardInfoModel]()
        for dict in list {
            models.append(OABoardInfoModel(dict: dict))
        }
        return models
    }
}
