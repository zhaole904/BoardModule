//
//  OABoardSwitchOrganViewController.swift
//  BoardModule
//
...
class OABoardSwitchOrganViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    @objc func loadPageData() -> () {
        ...
        if (branchCode.count == 0) {
            self.showRequestFailureViewWithTitle(title: "暂无数据", imageName: "OABoardUI.bundle/NoData1")
...
//                    let bodyArr = dataDic["body"] as! [[String: AnyObject]]
                    ...
                    if (code == "Y") {
                        let modelArr = OABoardInfoModel.listToModel(list: bodyArr as! [[String : AnyObject]])
                        self.dataArr = NSMutableArray.init(array: modelArr)
                        
//                        for dict in bodyArr! {
//                            let model = OABoardInfoModel.dictToModel(dic: dict)
...
//                        }
                    ...
                    self.showRequestFailureViewWithTitle(title: KgetResultMessage(data: resultData?.data as! NSDictionary), imageName: "OABoardUI.bundle/NoNetwork")
                }
            } else { //result=nil;
                self.showRequestFailureViewWithTitle(title: (resultData?.message!)!, imageName: "OABoardUI.bundle/NoNetwork")
            }
            self.tableView?.reloadData()
            self.tableView?.mj_header.endRefreshing()
        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if (self.dataArr.count > 0) {
            let model = self.dataArr[section] as! OABoardInfoModel
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = self.dataArr[indexPath.section] as! OABoardInfoModel
        if indexPath.row == 0 {
